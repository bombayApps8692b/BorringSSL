...
#include "third_party/boringssl/src/crypto/fipsmodule/rand/internal.h"
...
//CTR_DRBG_STATE ctr_drbg_state;
//
//JNIEXPORT jstring JNICALL
//Java_org_signal_aesgcmprovider_OpenSSLApiLibrary_getDRBGRandomNumber(JNIEnv *env, jobject thiz, jbyteArray seed) {
//
//    uint8_t random_number[16];
//
//    if (CTR_DRBG_init(&ctr_drbg_state, seed, sizeof(seed), NULL) != 1) {
//        printf("Error initializing CTR_DRBG.\n");
//        return (*env)->NewStringUTF(env, "Error initializing CTR_DRBG.\n");
//    }
//
//    if (CTR_DRBG_generate(&ctr_drbg_state, random_number, sizeof(random_number), NULL, 0) != 1) {
//        printf("Error generating random number.\n");
//        return (*env)->NewStringUTF(env, "Error generating random number.\n");
//    }
//
//    return (*env)->NewString(env, random_number, sizeof(random_number));
////    return (*env)->NewIntArray(env, sizeof(random_number));
//}


// Declare a static variable to hold the singleton instance
static CTR_DRBG_STATE ctr_drbg_state;

JNIEXPORT jstring JNICALL
Java_org_signal_aesgcmprovider_OpenSSLApiLibrary_getDRBGRandomNumber(JNIEnv *env, jobject thiz, jbyteArray seed) {
    int8_t random_number[16];
...
//    char buf[100]; // assumed large enough to cope with result
//
//    sprintf(buf, "%d", random_number);  // error checking omitted
//
